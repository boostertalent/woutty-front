...
> = {
  ...
  campaign_validated: {
    subject: 'Votre participation à la campagne est confirmée',
    body: (m) =>
      `L’assignation pour la campagne "${m.campaign_title}" (${m.brand_name ?? 'marque'}) a été validée. Connectez-vous pour continuer.`,
  },
  campaign_declined: {
    subject: 'Assignation non retenue',
    body: (m) =>
      `L’assignation pour "${m.campaign_title ?? 'votre campagne'}" n’a pas été retenue.${m.decline_reason ? ` Motif : ${m.decline_reason}` : ''}`,
  },
  creator_accepted: {
    subject: 'Un créateur a accepté la campagne',
    body: (m) =>
      `${m.creator_name ?? 'Un créateur'} a accepté la campagne "${m.campaign_title}".`,
  },
  creator_declined: {
    subject: 'Un créateur a décliné la campagne',
    body: (m) =>
      `${m.creator_name ?? 'Un créateur'} a décliné la campagne "${m.campaign_title}".${m.message ? ` Motif : ${m.message}` : ''}`,
  },
  content_submitted: {
