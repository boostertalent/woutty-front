...
export default function ForgotPasswordPage() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/creators/auth/reset-password`,
    });
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-[#FDFDFD] px-4 py-12 overflow-hidden font-sans">
      ...
      >
        ...
        <div className="text-center space-y-3">
          <Link href="/creators/auth/login" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-400 hover:text-black transition-colors mb-4">
