  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        ...
        // 2.  cherche dans la table 'createurs'
        ...
        //  On cherche dans la table 'marques'
        ...
        // 4. Si l'utilisateur n'est dans aucune des deux tables
        setErrorMsg("Compte introuvable dans nos bases Créateur ou Marque.");
      }
    } catch (error: any) {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          // Note: Le callback devra gérer la redirection vers la bonne table
      ...
      >
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                type="email" 
                required
                className="w-full rounded-2xl border border-gray-200 bg-gray-50/30 pl-12 pr-5 py-3.5 focus:border-[#ceaf4a] focus:bg-white outline-none transition-all"
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                required
                className="w-full rounded-2xl border border-gray-200 bg-gray-50/30 px-5 py-3.5 focus:border-[#ceaf4a] focus:bg-white outline-none transition-all"
                placeholder="••••••••"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#ceaf4a] text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-[#ceaf4a]/20 hover:bg-[#b8962f] disabled:opacity-50 active:scale-[0.98] flex items-center justify-center gap-2 transition-all"
          >
...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (!authError && authData.user) {
      ...
      // 2. VÉRIFICATION : Existe-t-il déjà ?
      // On cherche d'abord dans 'createurs'
      const { data: isCreator } = await supabase.from('createurs').select('id').eq('id', user.id).maybeSingle();
      
      if (isCreator) {
        targetTable = 'createurs';
        redirectPath = '/creators/dashboard';
      } else {
        // Sinon on cherche dans 'marques'
        const { data: isBrand } = await supabase.from('marques').select('id').eq('id', user.id).maybeSingle();
        if (isBrand) {
          targetTable = 'marques';
      ...
      if (!targetTable) {
        // S'il n'est nulle part, on utilise le roleFromUrl pour décider
        targetTable = roleFromUrl === 'brand' ? 'marques' : 'createurs';
        ...
        const { error: upsertError } = await supabase
          ...
          .upsert({
            ...
            // On peut ajouter un champ 'created_at' si ton camarade l'a prévu
          }, { onConflict: 'id' });
